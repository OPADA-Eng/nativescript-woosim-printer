...
export class WoosimPrinter extends Common {
    ...
    public foriegn_words_in_text(text) {
    ...
    printTextFile(file) {
        ...
        file.readText()
            .then((res) => {
                ...
                if (this.foriegn_words_in_text(res).length > 0) {
        ...
        // try {
        //     let data = [inStream.available()];
        //     while (inStream.read(data) != -1) {
        //         this.sendData(data);
        //     }
        // } catch (e) {
        //     console.log("sample 2inch receipt print fail.", e);
        // } finally {
        //     if (inStream != null)
        //         try {
        //             inStream.close();
    ...
    public printImg(bitmap, setExtraPaddingAfterPrint = true, XResol = 200, YResol = 200, startX = 0, startY = 0) {
        ...
        try {
            ...
            let data = com.woosim.printer.WoosimImage.printBitmap(0, 0, bitmap.getWidth(), bitmap.getHeight(), bitmap);
            if (this.debug)
    ...
    public print(string: string) {
        ...
        if (string == null)
            ...
            try {
                // text = new com.woosim.printer.WoosimArabic.encodePFB(string + '\n \n')
                let text = null;
                // console.log(typeof res);
                if (this.foriegn_words_in_text(string).length > 0) {
                    let arabic864 = new com.ganesh.intermecarabic.Arabic864();
                ...
                // java.Encoding.UTF32.GetBytes(new java.lang.String(string + '\n \n'))
                // text = this.unpack(string + '\n \n');

                // console.log(text.toString());
                let byteStream = new java.io.ByteArrayOutputStream();
                // byteStream.write(com.woosim.printer.WoosimCmd.setTextStyle(false, false, false, 3, 3));
                ...
                // byteStream.write(com.woosim.printer.WoosimCmd.setTextAlign(com.woosim.printer.WoosimCmd.ALIGN_CENTER));
                if (text != null) byteStream.write(text);
                console.log("byteStream >> ", byteStream.toString());
                byteStream.write(com.woosim.printer.WoosimCmd.printData());
                // this.sendData(arabic);
                this.sendData(com.woosim.printer.WoosimCmd.initPrinter());
                this.sendData(<any>byteStream.toByteArray());
                // this.sendData(com.woosim.printer.WoosimCmd.queryDeviceVersion());
            } catch (e) {
                console.log(e)
            }
        }
        ...
        // let text: any[] = this.unpack(string);
        // console.dir(text)
        // let byteStream = new java.io.ByteArrayOutputStream();
        // byteStream.write(com.woosim.printer.WoosimCmd.setTextStyle(false, false, false, 2, 2));
        // byteStream.write(com.woosim.printer.WoosimCmd.setTextAlign(com.woosim.printer.WoosimCmd.ALIGN_LEFT));
        // if (text != null)
        //     byteStream.write(text);
        // console.log("string byte: ", byteStream.toString())
        // byteStream.write(com.woosim.printer.WoosimCmd.printData());
        // this.sendData(com.woosim.printer.WoosimCmd.initPrinter());
        // this.sendData(<any>byteStream.toByteArray());
